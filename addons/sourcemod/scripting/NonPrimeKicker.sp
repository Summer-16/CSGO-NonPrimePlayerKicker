...
public void OnPluginStart() {
    g_cHourLevelEnabled = CreateConVar("sm_npmk_hourlevel_enabled", "1", "Should plugin check for minimun csgo hours for non licenced user");
...
public void OnClientPutInServer(int client) {
    ...
    if (!IsFakeClient(client)) {
        int hoursEnabled = g_cHourLevelEnabled.IntValue;
        char message[512];
        ...
        if (CheckCommandAccess(client, "BypassPremiumCheck", ADMFLAG_RESERVATION, true)) {
            ...
            if (k_EUserHasLicenseResultDoesNotHaveLicense == SteamWorks_HasLicenseForApp(client, 624820)) {
                if (hoursEnabled) {
                    ...
                    PrintToServer("[SUMMER-SOLDIER, NonPrimeKicker]==> Unlicenced Clinet and Plugin's hour check is disabled, Kicking the client");
...
public int RequestHours_OnHTTPResponse(Handle request, bool bFailure, bool bRequestSuccessful, EHTTPStatusCode eStatusCode, int userid) {
    ...
    if (minimumhours != 0) {
        if (totalPlayedTime < minimumhours) {
            PrintToServer("[SUMMER-SOLDIER, NonPrimeKicker]==> Unlicenced Clinet and Also not have enough CSGO play hours, Kicked");
    ...
    if (tagEnabled) {
        PrintToServer("[SUMMER-SOLDIER, NonPrimeKicker]==> Unlicenced Clinet but has enough play hours, Allowed in server with non prime tag");
